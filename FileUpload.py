...
class FileUpload(object):
    ...
    # allowed_extensions(type is list):the file has the extension is allowed
    # to upload

    def __init__(self, upload_folder, allowed_extensions={'gif', 'png', 'jpg', 'txt', 'jpeg', 'rar', 'mp4'}):
        super(FileUpload, self).__init__()
        self.upload_folder = os.path.join(os.getcwd(),upload_folder)
        ...
        if request.method == "POST":
            if request.headers.get('upload_resume'):
                ...
                filename = urllib.parse.unquote(
                    request.headers.get('upload_name'))
                file = os.path.join(self.upload_folder, filename)
                ...
                try:
                    ...
                    return '{"status":"error","reason":"server gets an error","detail":"' + str(e) + '"}'

        if request.method == "GET":
            size = int(request.args.get('size'))
            file = os.path.join(self.upload_folder, request.args.get('name'))
